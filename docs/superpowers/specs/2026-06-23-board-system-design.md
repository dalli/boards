...
- **공지사항(NOTICE)**: 관리자만 작성, 읽기는 게시판의 `read_visibility`(PUBLIC/AUTHENTICATED)에 따름(E-04)
- **일반(GENERAL)**: 인증 사용자가 쓰기, 읽기는 `read_visibility`에 따름. 첨부파일 업로드(백엔드 경유)/다운로드(presigned GET)
- **이미지(IMAGE)**: 게시물당 1개 이상 이미지 첨부 필수(생성·수정·삭제 전구간 강제, E-01), 썸네일 카드 그리드 → 라이트박스 원본 보기
...
- 시크릿: JWT secret, DB/MinIO 자격증명은 비커밋 시크릿(§4 보호 대상). prod는 런타임 시크릿 주입(deployment.md), 어떤 환경에서도 `.env` 평문 커밋 금지.
...
> **확정 AC는 [plan.md](../../../plan.md) v2의 "수락 기준 — 측정 가능 형태"를 따른다**(P-02). 아래는 초기 개요이며, 검증은 plan.md의 측정 가능한 AC1~AC9로 수행한다.

- [ ] AC1. USER/ADMIN 역할 분리 및 JWT 인증이 동작한다.
- [ ] AC2. ADMIN이 NOTICE/GENERAL/IMAGE 게시판을 생성할 수 있다.
- [ ] AC3. NOTICE는 ADMIN만 작성, USER는 읽기만 가능하다.
- [ ] AC4. GENERAL은 인증 사용자가 글 작성 + 첨부 업로드, 읽기·첨부 다운로드는 Board.read_visibility(PUBLIC/AUTHENTICATED)에 따른다(E-04). 확정 AC는 plan.md AC4.
